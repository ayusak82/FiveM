...
local function OpenPaymentMethodMenu(invoice)
    ...
    if invoice.status == 'seized' and invoice.paid_amount and invoice.paid_amount > 0 then
...
OpenPaymentMenu = function()
    ...
    for _, invoice in ipairs(pendingInvoices) do
        ...
        -- 残額を計算
        local remainingAmount = invoice.total_amount - (invoice.paid_amount or 0)
        local amountLabel = remainingAmount
        local amountDescription = '支払額: $' .. remainingAmount
        
        if invoice.status == 'seized' and invoice.paid_amount and invoice.paid_amount > 0 then
            amountDescription = string.format('元の金額: $%s\n既払額: $%s\n残額: $%s', 
                invoice.total_amount, invoice.paid_amount, remainingAmount)
        end
        ...
        table.insert(options, {
            ...
            metadata = {
                ...
                {label = invoice.status == 'seized' and invoice.paid_amount and invoice.paid_amount > 0 and '残額' or '金額', value = '$' .. remainingAmount},
            ...
            onSelect = function()
                ...
                if invoice.status == 'seized' then
                    if invoice.paid_amount and invoice.paid_amount > 0 then
...
local function OpenBossInvoiceMenu()
    ...
    for _, invoice in ipairs(invoices) do
        ...
        -- 残額を計算
        local remainingAmount = invoice.total_amount - (invoice.paid_amount or 0)
        local amount_text = string.format('$%s', remainingAmount)
        
        if invoice.status == 'seized' and invoice.paid_amount and invoice.paid_amount > 0 then
            amount_text = string.format('$%s (残額)', remainingAmount)
        end
